...
def precompute_data_stack_mode(points, lengths, num_stages, voxel_size, radius, neighbor_limits):
    ...
    keypoint_ref = iss(ref_input, gamma21=0.6, gamma32=0.6, KDTree_radius=3.0, NMS_radius=3.0, max_num=5000)
    keypoint_src = iss(src_input, gamma21=0.6, gamma32=0.6, KDTree_radius=3.0, NMS_radius=3.0, max_num=5000)
    ...
    # non-salient point 
    ref_len_o=lengths_list[-1][0]
    src_len_o=lengths_list[-1][1]
    ref_points_c_o=points_list[-1][:ref_len_o]
    src_points_c_o=points_list[-1][ref_len_o:]
    
    dist_matrix = torch.sqrt(pairwise_distance(ref_points_c_o,keypoint_ref))
    dist = torch.topk(dist_matrix,k=1,dim=-1,largest=False)[0]
    ref_mask_s = (dist > sigmma).view(-1) 

    #hybridpoint for ref 
    ref_points_c = torch.cat((keypoint_ref, ref_points_c_o[ref_mask_s]), dim=0)

    dist_matrix = torch.sqrt(pairwise_distance(src_points_c_o,keypoint_src))
    dist = torch.topk(dist_matrix,k=1,dim=-1,largest=False)[0]
    src_mask_s = (dist > sigmma).view(-1)  

    #hybridpoint for src
    src_points_c = torch.cat((keypoint_src, src_points_c_o[src_mask_s]), dim=0)

    #replace original superpoint by hybridpoint
    lengths_list[-1][0] = ref_points_c.size(0)
    lengths_list[-1][1] = src_points_c.size(0)
